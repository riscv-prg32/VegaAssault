...
## Manual forms and continuous-video revision (2026-09-03)

The updated scene fixture (`Manual forms QA`, 28456 code bytes / 32276 executable bytes) exercised DOWN+A detachment and UP+A recombination in the sky, water, tunnel and Moon environments. Inspected framebuffers show detached craft, robots and recombined flight; the Moon retains its three extra escorts. The approximately 50-second mono capture contained 2216466 PCM bytes, peak 8177, without clipping or a guest panic. Evidence is under `build/qemu-manual/`. This remains injected target-scene evidence, not an input-only playthrough.

The restored playable release loaded 28028 code bytes, 31844 executable bytes and 2284 audio bytes; its package is 30412 bytes. Its launch/combat/pause/resume smoke run completed without a guest panic (1502928 mono PCM bytes, peak 10816, no clipping). Release SHA-256: `34f49f8eb5250895bf5bddabfbd66756cb3b17edc9fe13b8ced28a8c059f55af`. Native tests cover both chords, speed, all weapon damage, detached support, pause and full pools in every stage. Physical ESP32-C6 testing remains outstanding.

The revised 30.00-second video has a separate uninterrupted stage-one soundtrack, peak 5696 before encoding. The music event grid remains continuous and audio is present around every scene cut. Video and audio provenance are documented in `demo.md`; game stage music is unchanged.
