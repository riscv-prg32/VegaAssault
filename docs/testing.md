...
## Five-stage campaign validation (2026-09-03)

C99 warnings-as-errors, educational comments, ASan/UBSan gameplay regressions and all ten Python audio/Store tests pass. The extended suite checks the exact launch duration, support introductions/docking, distinct craft selection, all five music phrases (including wrapped indices), bounded bosses, stationary Moon base, lethal final-hit precedence and every boss-to-stage/victory transition. Native visual fixtures cover all terrain/form combinations and the opening/finale; these explicitly set state and do not constitute an input-only campaign playthrough.

The portable builder reports 26624 code/data bytes, 30440 total executable bytes (limit 32768), 2284 audio bytes and a 29008-byte package (limit 131072). Physical ESP32-C6 verification and full target campaign playtesting remain outstanding; no USB serial board was present during this run. QEMU observations are recorded separately in `docs/qemu.md`. Existing stereo/mono software-model and packer tests cover reused audio; hardware stereo listening is still pending.

The first QEMU pass exposed a load-access fault on entering the Earth introduction: a compiler-generated text pointer table retained the link address. The fixed character matrix and explicit support selection remove absolute data-pointer relocations. An additional RV32 GCC C99 `-Wall -Wextra -Werror -Os -mcmodel=medany -msmall-data-limit=0` compile passed; `readelf -r` found no `R_RISCV_32` entries in the game object. The final Store ZIP passed CRC checks and its cartridge matched the build byte-for-byte; metadata describes version 1.3.0 and five missions.
