...
## Five-mission campaign fixtures

![Native launch, five missions, Special Cosmo, lunar forms and ending fixtures](../assets/campaign-scenes.png)

Generated by `python3 tools/capture_campaign.py` from the real cartridge drawing code and upstream PRG32 software renderer. Scenes and launch/docking timers are explicitly selected by the host test harness, so this sheet documents appearance rather than a completed playthrough. QEMU observations for the built cartridge are recorded in `qemu.md`.
